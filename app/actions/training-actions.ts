...
import { loadTrainingDocuments, getTrainingDocumentById, TrainingDocument, createMockTrainingData } from '@/lib/simple-pdf-processor';
...
export async function refreshTrainingData(): Promise<{ success: boolean; count: number; message: string }> {
  try {
    // Use mock training data instead of processing PDFs
    const documents = await createMockTrainingData();
    return {
      success: true,
      count: documents.length,
      message: `Successfully created ${documents.length} mock training documents`
    };
  } catch (error: any) {
    ...
    return {
      success: false,
      count: 0,
      message: error.message || 'An error occurred while creating mock training data'
    };
  }
}
